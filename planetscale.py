...
class PlanetScaleCheck(OpenMetricsBaseCheck):
    DEFAULT_METRIC_LIMIT = 0 # By default, collect all metrics from discovered endpoints
    ...
    def __init__(self, name, init_config, instances):
        ...
        for inst in instances:
            ...
            init_inst.setdefault('prometheus_url', 'http://localhost:1/dummy') # Dummy URL
    ...
    def check(self, instance):
        ...
        if not org_id:
            raise ConfigurationError("Missing 'planetscale_organization' in instance configuration.")
        if not token_id:
            raise ConfigurationError("Missing 'ps_service_token_id' in instance configuration.")
        if not token_secret:
            raise ConfigurationError("Missing 'ps_service_token_secret' in instance configuration.")

        # Get optional configuration
        req_timeout = instance.get('timeout', self.init_config.get('timeout', 10)) # Use instance timeout, then init_config, then default 10
        ssl_verify = instance.get('ssl_verify', True)

        # Construct API URL
        api_url = f"https://api.planetscale.com/v1/organizations/{org_id}/metrics"
        headers = {'Accept': 'application/json', 'Authorization': f"{token_id}:{token_secret}"}
        ...
        self.service_check("planetscale.api.can_connect", AgentCheck.OK, tags=[f"planetscale_org:{org_id}"])

        # Process each discovered target using the OpenMetricsBaseCheck logic
        for target_config in targets:
            if not target_config.get('targets'):
                self.log.warning(f"Skipping target due to missing 'targets' field: {target_config}")
                continue

            # Create a dynamic instance configuration for this specific target
            dynamic_instance = deepcopy(instance) # Start with a copy of the original instance config

            # --- Construct the full Prometheus URL ---
            base_target = target_config['targets'][0] # e.g., "hostname:port" or "http://hostname:port"
            ...
            query_string = requests.compat.urlencode(url_params) # Use requests' helper for encoding
            ...
            self.log.debug(f"Constructed scrape URL: {final_url} from target: {target_config}")
            ...
            for key, value in discovered_labels.items():
                if not key.startswith('__'): # Exclude special labels like __metrics_path__, __param_*
            ...
            try:
                ...
                self.log.error(f"Error processing dynamic instance {dynamic_instance.get('prometheus_url')}: {e}")
                # Add a service check for the specific target failure
                target_tags = dynamic_instance.get('tags', [])
                target_tags.append(f"prometheus_url:{dynamic_instance.get('prometheus_url')}")
                self.service_check("planetscale.target.can_scrape", AgentCheck.CRITICAL, message=str(e), tags=target_tags)
